...
import { updateCloudFront } from './cloudFront_helpers'
import {
    addSeperatorToPath,
    addTrailingS3Sep,
...
module.exports = class S3Plugin {
    ...
    isIncludeAndNotExclude(file) {
        var isExclude,
            isInclude, { include, exclude } = this.options
    ...
    chngeOriginPath() {
        const { clientConfig, cloudfrontInvalidateOptions, basePath } = this
        if (cloudfrontInvalidateOptions.DistributionId) {
            const { accessKeyId, secretAccessKey } = clientConfig.s3Options
            return updateCloudFront(cloudfrontInvalidateOptions.DistributionId, basePath, accessKeyId, secretAccessKey)
        } else {
            return Promise.resolve(null);
        }
        return updateCloudFront()
    }
}